...
| **CUPED Variance Reduction** | `cuped_variance_reduction` | Streaming-sessions A/B test walkthrough — using pre-experiment covariate data to strip predictable noise out of the outcome metric, with a step-by-step theta/variance-reduction calculation showing how CUPED tightens confidence intervals without changing the estimated effect. |
| **Synthetic Control** | `synthetic_control` | Ride-sharing market-pilot case study — building a weighted "synthetic" counterfactual from untreated donor markets (via grid search and a general SLSQP optimizer) to estimate a treatment effect with no individual-level randomisation, plus the pre-period fit check and placebo test that confirm the counterfactual is trustworthy. |
| **Sequential Testing** | `sequential_testing` | How to peek at a running experiment without breaking it — a single no-real-effect experiment showing the p-value "random walk" under daily monitoring, plus a 20,000-run Monte Carlo comparing four stopping strategies to show how much peeking inflates the false-positive rate and how a Bonferroni correction brings it back under control. |
| **Novelty Effects** | `novelty_effects` | Mobile-game feature A/B test case study (gaming industry) — a Daily Spin Wheel's 28% day-1 lift is mostly novelty curiosity decaying toward a true 5% steady-state effect, shown via a 30-day decay curve and a faster fix: comparing new players (no "before" state to be curious about) against existing players to get an unbiased read in a single day instead of thirty. |
